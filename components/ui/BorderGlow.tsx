...
type GlowVars = Record<
  "--glow-color" | "--glow-color-60" | "--glow-color-50" | "--glow-color-40" | "--glow-color-30" | "--glow-color-20" | "--glow-color-10",
  string
>;

type GradientVars = Record<
...
function buildGlowVars(glowColor: string, intensity: number): GlowVars {
  ...
  keys.forEach((key, i) => {
    vars[`--glow-color${key}` as keyof GlowVars] = `hsl(${base} / ${Math.min(opacities[i] * intensity, 100)}%)`;
  });
  return vars;
}
